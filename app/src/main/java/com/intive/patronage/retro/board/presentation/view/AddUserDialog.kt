        ...
        CoroutineScope(Dispatchers.Main).launch {
            flow?.collect { result ->
                if (result.isNotEmpty() && result[0] == "") {
                    ...
                    val editText = binding.newUserEmail.editText?.text.toString()
                    var emails = mutableListOf<String>()

                    for (i in result.indices) {
                        if (result[i].length >= editText.length && editText == result[i].substring(0, editText.length)) {
                            emails.add(result[i])
                        }
                    }
                    ...
                    if (emails.isEmpty()) {
                        ...
                        binding.addEmailButton.isEnabled = editText in emails
                    }
                }
            }
        }
